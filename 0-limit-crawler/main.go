...
func Crawl(url string, depth int, wg *sync.WaitGroup, wait <-chan time.Time) {
	defer wg.Done()
...
func main() {
	...
	wg.Add(1)
	wait := time.Tick(time.Second)
	Crawl("http://golang.org/", 4, &wg, wait)
	wg.Wait()
}
